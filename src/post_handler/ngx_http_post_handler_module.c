...
static char *ngx_http_post_handler_module_name(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    DD("module name: %s", olcf->ph_module_name.data);//parrot
    DD("config file: %s", olcf->ph_config_file.data);//5

#ifdef WIN32
    snprintf(so_name, MAX_SO_NAME, "%s.dll", olcf->ph_module_name.data);
    dl_lib = LoadLibrary(so_name);
    if (!dl_lib) {
        fprintf(stderr, "%d\n", (int)GetLastError());
        return NGX_CONF_ERROR;
    }
    post_handler_worker = GetProcAddress(dl_lib, (char *) olcf->ph_module_name.data);
    if(post_handler_worker == NULL){
        fprintf(stderr, "%d\n", (int)GetLastError());
        return NGX_CONF_ERROR;
    }

   if(error==NULL){}  //为了兼容linux
...
/*获取tcp连接对象阶段，会设置读事件的回调为ngx_http_init_request，如果此时有客户端发起了http请求，则这个回调会被调用。用于建立一个ngx_http_request_t这样的http结构。在这个阶段只是创建一个http请求结构，并创建一个接收缓冲区，一个发送缓冲区等，并没有实际处理来自客户端的数据。
————————————————
版权声明：本文为CSDN博主「ApeLife」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/ApeLife/article/details/54429582
*/
ngx_int_t
...
ngx_http_post_handler_handler(ngx_http_request_t *r)
{
    ...
    // output header
    ngx_str_t res_type = ngx_string("text/plain");
    ngx_str_t response = ngx_string("this is response.");
    
    r->headers_out.content_type = res_type;
    r->headers_out.content_type.len = sizeof("text/plain") - 1;
    r->headers_out.content_type.data = response.data;
